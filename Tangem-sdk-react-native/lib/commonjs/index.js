...
function getJsonRequest(method = 'scan', params = {}) {
  return {
    jsonrpc: '2.0',
    id: '1',
    method,
...
function convertRequest(params) {
...
function execCommand(method = 'scan', params = {}, cardId, initialMessage) {
  return execJsonRPCRequest(getJsonRequest(method, params), cardId, initialMessage);
}

async function execJsonRPCRequest(jsonRequest = {}, cardId, initialMessage) {
  const request = {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const parseResponse = typeof response === 'object' ? response : JSON.parse(response);
...
const tangemSdk = {
   ...
   */
  scanCard: initialMessage => execCommand('scan', {}, undefined, initialMessage),
   ...
   */
  signHash: (hash, walletPublicKey, cardId, hdPath, initialMessage) => execCommand('sign_hash', {
    walletPublicKey,
    hdPath,
    hash
  }, cardId, initialMessage),
   ...
   * @returns {Promise<SignHashResponse>} response
   */
  signHashes: (hashes, walletPublicKey, cardId, hdPath, initialMessage) => execCommand('sign_hashes', {
    walletPublicKey,
    hdPath,
    hashes
  }, cardId, initialMessage),
   ...
   */
  createWallet: (curve, cardId, initialMessage) => execCommand('create_wallet', {
    curve
  }, cardId, initialMessage),
   ...
   */
  purgeWallet: (walletPublicKey, cardId, initialMessage) => execCommand('purge_wallet', {
    walletPublicKey
  }, cardId, initialMessage),
   ...
   * @returns {Promise<SuccessResponse>}
   */
  setAccessCode: (accessCode, cardId, initialMessage) => execCommand('set_accesscode', {
    accessCode
  }, cardId, initialMessage),
   ...
   * @returns {Promise<SuccessResponse>}
   */
  setPasscode: (passcode, cardId, initialMessage) => execCommand('set_passcode', {
    passcode
  }, cardId, initialMessage),
   ...
   */
  resetUserCodes: (cardId, initialMessage) => execCommand('reset_usercodes', {}, cardId, initialMessage),
   ...
   */
  readFiles: (readPrivateFiles, fileName, walletPublicKey, cardId, initialMessage) => execCommand('read_files', {
    readPrivateFiles,
    fileName,
    walletPublicKey
  }, cardId, initialMessage),
   ...
   */
  writeFiles: (files, cardId, initialMessage) => execCommand('write_files', {
    files
  }, cardId, initialMessage),
   ...
   * @returns {Promise<SuccessResponse>}
   */
  deleteFiles: (indices, cardId, initialMessage) => execCommand('delete_files', {
    indices
  }, cardId, initialMessage),
   ...
   * @returns {Promise<SuccessResponse>}
   */
  changeFilesSettings: (changes, cardId, initialMessage) => execCommand('change_file_settings', {
    changes
  }, cardId, initialMessage),
   ...
   */
  prepareHashes: (cardId, fileData, fileCounter, fileName, privateKey) => RNTangemSdk.prepareHashes(convertRequest({
    cardId,
    fileData,
    fileCounter,
    fileName,
  ...
  runJSONRPCRequest: (JSONRPCRequest, cardId, initialMessage) => execJsonRPCRequest(JSONRPCRequest, cardId, initialMessage),
  ...
  on: (eventName, handler) => {
    if (_reactNative.Platform.OS === 'android') {
      _reactNative.DeviceEventEmitter.addListener(eventName, handler);
    }
  },
  ...
  removeListener: (eventName, handler) => {
    if (_reactNative.Platform.OS === 'android') {
      _reactNative.DeviceEventEmitter.removeListener(eventName, handler);
    }
  }
}; // export all types

// export module
var _default = tangemSdk;
